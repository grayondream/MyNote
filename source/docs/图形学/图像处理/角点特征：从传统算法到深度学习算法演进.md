...
### 3.4 SIFT 角点检测
#### 3.4.1 原理
&emsp;&emsp;SIFT（Scale-Invariant Feature Transform）是一种计算机视觉算法，用于检测、描述和匹配图像中的局部特征，由David Lowe于 1999 年发明。SIFT的工作流程包括四个主要步骤：首先，通过高斯模糊生成不同尺度的图像，检测尺度空间中的极值点；其次，对这些关键点进行精确定位，剔除低对比度和边缘响应强的点；接着，根据关键点周围的梯度方向分配方向，以实现旋转不变性；最后，生成128维的特征描述子，有效表示关键点的局部信息。SIFT对光照、旋转和尺度变化具有良好的不变性，但计算复杂度较高，速度相对较慢。
**尺度空间极值检测**
&emsp;&emsp;尺度空间极值检测旨在识别图像中具有显著特征的点。
